<#
.SYNOPSIS
Mirrors the seven core Fabric ingester notebooks from `deployment/1. Fabric/notebooks/`
into every self-contained location under `3. Fabric Extended/`.

.DESCRIPTION
The source of truth for the core ingesters is `deployment/1. Fabric/notebooks/`. To keep
each `3. Fabric Extended/*` add-on downloadable-and-runnable in isolation, we
duplicate those notebooks into:

  deployment/3. Fabric Extended/_shared/notebooks/                       (documentation copy)
  deployment/3. Fabric Extended/Fabric + Copilot Studio/notebooks/_core/ (runnable copy)
  deployment/3. Fabric Extended/Fabric + M365/notebooks/_core/           (runnable copy)

Run this after editing any file in `deployment/1. Fabric/notebooks/`.
...
$source   = Join-Path $repoRoot 'deployment\1. Fabric\notebooks'

$destinations = @(
    (Join-Path $repoRoot 'deployment\3. Fabric Extended\_shared\notebooks'),
    (Join-Path $repoRoot 'deployment\3. Fabric Extended\Fabric + Copilot Studio\notebooks\_core'),
    (Join-Path $repoRoot 'deployment\3. Fabric Extended\Fabric + M365\notebooks\_core')
)
